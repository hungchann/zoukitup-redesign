...
const DJs: React.FC = () => {
  ...
  const nextSlide = React.useCallback(() => {
    setCurrentIndex((prev) => {
      const currentMaxIndex = Math.max(0, djs.length - (window.innerWidth < 768 ? 1 : 3));
                ...
                <div key={dj.id} className="group text-center flex-shrink-0 px-4" style={{ width: `${100 / itemsPerView}%` }}>
                  ...
                  {dj.link && (
                    <a
                      ...
                      className="inline-flex items-center gap-2 px-4 py-2 mb-3 bg-white border border-logo-purple-2/30 text-logo-purple-2 hover:bg-logo-purple-2 hover:text-white transition-all duration-300 text-sm uppercase tracking-widest group"
                    >
                      <Music size={16} />
